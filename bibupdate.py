...
bibupdate_version=r'''
%(prog)s, version {version}: {description}
{license}
'''.format(**meta_data)

# The following __doc__ string is used by setup.py to automatically generate the
# README file. Doing it this way has the advantage that we don't need to
...
# import and use fuzzywuzzy.fuzz if we can....
try:
    ...
    def good_match(one,two):
        return True
    # would prefer to use warning() but this is not defined yet...
    print(r'''# bibupdate warning: python module the fuzzywuzzy not installed so
# bibupdate is unable to reliably check that titles match. Consider 
# installing fuzzywuzzy using: pip install fuzzywuzzy''')
